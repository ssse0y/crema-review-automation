...
captureTest.addEventListener("click", async () => {
  ...
  status.textContent = "첫 번째 리뷰의 캡처·시트 기록과 다운로드 알림을 테스트합니다…";
  const result = await chrome.runtime.sendMessage({type: "runCaptureTest"});
  if (!result?.ok) {
    renderRunStatus({lastRunStatus: "error", lastRunMessage: "캡처·시트·다운로드 알림 테스트를 시작하지 못했습니다.", lastRunDetail: result?.error || "알 수 없는 오류"});
    return;
  }
